...
WindowButton::WindowButton(){
    m_isEllipse = true;
	setTextColor(FCColor::rgb(200, 200, 200));
    FCSize size = {200, 200};
    setSize(size);
...
Long WindowButton::getPaintingBackColor(){
    ...
    if (m_style == WindowButtonStyle_Close){
        if (native->getPushedView() == this){
            return FCColor::rgb(255, 0, 0);
        }
        else if (native->getHoveredView() == this){
            return FCColor::rgb(255, 150, 150);
        }
        else{
            return FCColor::rgb(255, 80, 80);
        }
    }
    else if (m_style == WindowButtonStyle_Min){
        if (native->getPushedView() == this){
            return FCColor::rgb(0, 255, 0);
        }
        else if (native->getHoveredView() == this){
            return FCColor::rgb(150, 255, 150);
        }
        else{
            return FCColor::rgb(80, 255, 80);
        }
    }
    else{
        if (native->getPushedView() == this){
            return FCColor::rgb(255, 255, 0);
        }
        else if (native->getHoveredView() == this){
            return FCColor::rgb(255, 255, 150);
        }
        else{
            return FCColor::rgb(255, 255, 80);
        }
    }
}
